...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference('Status.count') do
      post statuses_url, params: { status: { is_final: @status.is_final, name: @status.name } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { is_final: @status.is_final, name: @status.name } }
